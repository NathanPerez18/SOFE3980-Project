...
public class BinaryControllerTest {
    ...
    @Test
    public void calculateWithoutOperator() throws Exception {
    this.mvc.perform(post("/calculate")
            .param("operand1", "1")
            .param("operand2", "1")) // No operator selected
        .andExpect(status().isOk())
        .andExpect(view().name("error")) // Assuming an error view is rendered
        .andExpect(model().attributeExists("errorMessage"));
}

@Test
public void submitWithEmptyOperand() throws Exception {
    this.mvc.perform(post("/calculate")
            .param("operand1", "") // First operand is empty
            .param("operator", "+")
            .param("operand2", "1")) // Second operand is valid
        .andExpect(status().isOk())
        .andExpect(view().name("error")) // Assuming an error view is rendered
        .andExpect(model().attributeExists("errorMessage")); // Check if an error message is present
}

@Test
public void calculateWithSingleOperand() throws Exception {
    this.mvc.perform(post("/calculate")
            .param("operand1", "1") // Only one operand is provided
            // No operator and second operand provided
            )
        .andExpect(status().isOk())
        .andExpect(view().name("error")) // Assuming an error view is rendered
        .andExpect(model().attributeExists("errorMessage")); // Check if an error message is present
